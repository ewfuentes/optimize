...
TEST(FunctionTest, test_function) {
  struct TestFunction : public Function<1> {
    double operator()(const Vector &x) { return x + 1.0; }
  };
  TestFunction x;
  EXPECT_EQ(x(1), 2.0);
}
...
TEST(DifferentiableFunctionTest, test_function) {
  struct TestFunction : public DifferentiableFunction<1> {
    double operator()(const double &x) { return x + 1.0; }

    double gradient(const double &x) { return 1.0; }
  };
  TestFunction x;
  EXPECT_EQ(x(1), 2.0);
  ...
  TEST(TwiceDifferentiableFunctionTest, test_function) {
    struct TestFunction : public TwiceDifferentiableFunction<1> {
      double operator()(const double &x) { return x*x + 2 * x + 1.0; }

      double gradient(const double &x) { return 2*x + 2; }

      double hessian(const double &x) {return 2; }
    };
    TestFunction x;
    EXPECT_EQ(x(1), 4.0);
